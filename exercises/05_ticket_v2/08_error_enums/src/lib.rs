...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    ...
    match ticket {
        ...
        Result::Err( TicketNewError::TitleError{err}) => panic!("{}",err),
        Result::Err( TicketNewError::DescriptionError{..}) => Ticket::new(title,"Description not provided".into(), status).unwrap()
    }
}
...
impl Ticket {
    pub fn new(
        ...
        if title.len() > 50 {
            return Err(TicketNewError::TitleError{ err:"Title cannot be longer than 50 characters".to_string() });
        }
        if description.is_empty() {
            return Err(TicketNewError::DescriptionError{ err:"Description cannot be empty".to_string() });
        }
        if description.len() > 500 {
            return Err(TicketNewError::DescriptionError{ err:"Description cannot be longer than 500 characters".to_string() });
        }
